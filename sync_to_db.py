# sync_to_db.py
# Lee los archivos Excel (ordenes + IXC) y popula tablero.db (SQLite)
# Ejecutar: python sync_to_db.py
# En Azure: este script corre cuando el Excel cambia (webhook/schedule)

import sys, io
sys.stdout = io.TextIOWrapper(sys.stdout.buffer, encoding='utf-8', errors='replace')
sys.stderr = io.TextIOWrapper(sys.stderr.buffer, encoding='utf-8', errors='replace')

import openpyxl, json, re, os, sqlite3, platform, tempfile, shutil
...
def leer_ixc_excel(path):
    if not os.path.exists(path):
        print(f"  INFO IXC Excel no encontrado"); return {}
...
def init_db(conn):
    ...
    CREATE TABLE IF NOT EXISTS ixc_totales (
    ...
    CREATE INDEX IF NOT EXISTS idx_ordenes_fn ON ordenes(fn);
    CREATE INDEX IF NOT EXISTS idx_ordenes_marca ON ordenes(marca);
    CREATE INDEX IF NOT EXISTS idx_ordenes_pais ON ordenes(pais);
    """)
    conn.commit()

def insertar_ordenes(conn, records_main, records_ss_raw):
    sin_stock_fechas = set(r['fo']+r['fn'] for r in records_ss_raw if es_sin_stock(r))
    conn.execute("DELETE FROM ordenes")
    filas = []
    for r in records_main:
        clave = r['fo'] + r['fn']
        ss = 1 if (es_sin_stock(r) or clave in sin_stock_fechas) else 0
        filas.append((
            r['fo'], r['fn'], r['com'], r['est'], r['pen'],
            r['marca'], r['pais'], r['dup'], r['ops'],
            r['venta'], r['ticket'], r['prov'], ss, 'main'
        ))
    # Registros ss que no están en main (solo de enero u otros archivos extra)
    fo_fn_main = set((r['fo'], r['fn']) for r in records_main)
    for r in records_ss_raw:
        if (r['fo'], r['fn']) not in fo_fn_main and es_sin_stock(r):
            filas.append((
                r['fo'], r['fn'], r['com'], r['est'], r['pen'],
                r['marca'], r['pais'], r['dup'], r['ops'],
                r['venta'], r['ticket'], r['prov'], 1, 'ss'
            ))
    conn.executemany(
        "INSERT INTO ordenes (fo,fn,com,est,pen,marca,pais,dup,ops,venta,ticket,prov,sin_stock,fuente) VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?,?)",
        filas
    )
    conn.commit()
    print(f"  -> {len(filas)} filas insertadas en ordenes")

def insertar_ixc(conn, ixc_data):
    ahora = datetime.now().isoformat()
    conn.executemany(
        "INSERT OR REPLACE INTO ixc_totales (fecha, total, actualizado) VALUES (?,?,?)",
        [(fecha, total, ahora) for fecha, total in ixc_data.items()]
    )
    conn.execute("INSERT OR REPLACE INTO meta (clave,valor) VALUES ('ultima_sync',?)", (ahora,))
    conn.commit()
    print(f"  -> {len(ixc_data)} fechas IXC insertadas")

# -- Main ---------------------------------------------------------------------
if __name__ == "__main__":
    print(f"\n{'='*52}")
    print(f"  Tablero GDI — Sync Excel → SQLite")
    ...
    print(f"RECORDS_SS raw: {len(records_ss_raw)} registros\n")

    if not records_main:
        print("WARNING Sin registros en archivo principal. Abortando."); exit(1)

    ixc_data = leer_ixc_excel(IXC_EXCEL)

    print(f"\nCreando/actualizando base de datos: {DB_PATH}")
    conn = sqlite3.connect(DB_PATH)
    init_db(conn)
    insertar_ordenes(conn, records_main, records_ss_raw)
    insertar_ixc(conn, ixc_data)
    conn.close()

    print(f"\n{'='*52}")
    print(f"  OK tablero.db actualizada correctamente")
    print(f"{'='*52}\n")
